...
derive that physical layout from the logical A/B shape.
"""
...
def tile_atom_to_shape_SF(tensor_or_shape, sf_vec_size,
    ...
    """Derive canonical SF storage from a logical A/B ``(mn, k, l)``.

    Logical K first contracts by ``sf_vec_size``; four scale values form
    each 32-bit block.  Flattened modes match the example's physical tensor:
    ``(32,4,rm,4,rk,l)``.
    """
    ...
    if len(shape) == 3:
        ...
        m, k, l = shape[0], shape[1], 1
    else:
        raise NotImplementedError(f"SF shape {shape}")
    k_sf = -(-k // int(sf_vec_size))
    rest_m = -(-m // 128)
    rest_k = -(-k_sf // 4)
    return L(((32, 4, rest_m), (4, rest_k), l),
             ((16, 4, rest_k * 512), (1, 512),
              rest_m * rest_k * 512))
